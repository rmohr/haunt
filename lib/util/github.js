...
var github = function (path, options, callback) {
    ...
    if (Object.keys(options).length) {
        if (method == 'get') path += '?' + querystring.stringify(options);
          ...
          req.body = JSON.stringify([ options.label ])
...
module.exports.getIssues = function (user, repo, since, page, issues, callback) {
    if (typeof page == 'function') {
        callback = page;
        issues   = [];
        page     = 1;
    ...
    github('/repos/:user/:repo/issues', options, function (err, result) {
        ...
        if (result.length === 50) module.exports.getIssues(user, repo, since, page + 1, issues, callback);
...
module.exports.untagIssue = function (user, repo, label, number, callback) {
    github('/repos/:user/:repo/issues/:number/labels/:label', {
        user: user,
        repo: repo,
        label: label,
        number: number,
