...
Repo.prototype.getIssues = function (callback) {
    ...
    github.getIssues(this.data.owner.login, this.data.name, this.options.since, function (err, issues) {
        if (err) return callback(err);
        ...
        this.issues = issues
            .map(function (issue) { return (!issue.pull_request || this.options.prasissue && this.options.issuesOnly) && issue }.bind(this))
